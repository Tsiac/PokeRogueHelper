...
pokemon_dict = {p['name'].lower(): p for p in pokemon_data}
...
def calculate_effectiveness(attacker_type, defender_types):
    return effectiveness_chart.get(attacker_type, {}).get(defender_types[0], 1) * (effectiveness_chart.get(attacker_type, {}).get(defender_types[1], 1) if len(defender_types) > 1 else 1)
...
def show_vulnerability_result():
    ...
    for row_index, dropdown_vars_set in enumerate(dropdown_vars):
        ...
        for col_index, atype in enumerate(types):
            effectiveness = calculate_effectiveness(atype, [type1, type2]) if type1 and type2 else calculate_effectiveness(atype, [type1]) if type1 else 1.0
            text, text_color = {
                4.0: ("4x", "#FF0000"),
                2.0: ("2x", "#8B0000"),
                0.5: ("½x", "#008000"),
...
def compare_rival_team():
    ...
    for i in range(6):
        ...
        tk.Entry(rival_frame, textvariable=name_var).grid(row=i, column=1, padx=5, pady=5)
    ...
    def calculate_preparedness():
        ...
        for row_index, dropdown_vars_set in enumerate(dropdown_vars):
            ...
            for move_var in dropdown_vars_set:
                ...
                if move_type:
                    for rival_name, rival_type1, rival_type2 in rival_team:
                        effectiveness = calculate_effectiveness(move_type, [t for t in (rival_type1, rival_type2) if t])
            ...
            for rival_name, rival_type1, rival_type2 in rival_team:
                rival_effectiveness = max(
                    calculate_effectiveness(rival_type1, [type1, type2]),
                    calculate_effectiveness(rival_type2, [type1, type2]) if rival_type2 else 0
                )
        ...
        def show_rival_vulnerability():
            ...
            for row_index, (rival_name, rival_type1, rival_type2) in enumerate(rival_team):
                ...
                for col_index, atype in enumerate(types):
                    effectiveness = calculate_effectiveness(atype, [rival_type1, rival_type2]) if rival_type2 else calculate_effectiveness(atype, [rival_type1])
                    text, text_color = {
                        4.0: ("4x", "#FFFFFF"),
                        2.0: ("2x", "#FFFFFF"),
                        0.5: ("½x", "#000000"),
